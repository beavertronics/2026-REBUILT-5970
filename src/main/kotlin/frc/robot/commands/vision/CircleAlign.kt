...
class CircleAlign(
    ...
    override fun execute() {
        ...
        val currentAngleToCenter = Drivetrain.swerveDrive.pose.vector2.angleTo(targetCenter())

        var rotationSpeed =
            rotationPID.calculate(Drivetrain.swerveDrive.pose.rotation.radians - angleProvider().asRadians)

        var circleSpeed =
            circlePID.calculate(
                MathUtil.angleModulus(
                    Drivetrain.swerveDrive.pose.vector2.angleTo(targetCenter()).asRadians -
                            angleProvider().asRadians
                )
            )
        var distanceSpeed =
            distancePID.calculate(
                Drivetrain.swerveDrive.pose.vector2.distance(targetCenter()) - desiredDistance().asMeters
            )
        println(Drivetrain.swerveDrive.pose.vector2.distance(targetCenter()) - desiredDistance().asMeters)
